...
class ThirdSession extends FunSpec with Matchers with SeveredStackTraces {
  ...
  describe("ReverserTrait") {
    ...
    describe("through SeqReverser") {
      it("should reverse the order of sequences") {
        ...
        assert(
          seqReverse.reverse(List("does", "it", "work")) === List("work",
                                                                  "it",
                                                                  "does"))
      }
    }
    ...
    class IntReverser extends ReverserTrait {
      ...
      def reverse(i: Int): Int =
        throw new IllegalStateException("Cannot reverse an int")
    }
  ...
  describe("A Blaster") {
    ...
    it(
      "should be able to load as many times as you like, but still only give three shots") {
