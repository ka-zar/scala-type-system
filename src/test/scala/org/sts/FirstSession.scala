...
class FirstSession extends FunSuite with Matchers with SeveredStackTraces {
  ...
  test("Can create an empty Car of a certain type with Car.empty[TYPE]") {
    ...
    // note that you should not be able to make a Car for anything that is not a subtype
  ...
  //
  // You should also be able to find out how many items are in the car at present, using a
  ...
  //
  // Suggest you hold on to items in a car with a Vector, as this makes updating items at the end
  // and retrieving an item by index easy and efficient.

  test(
    "Empty Car should store items of the right type and access them by index") {
  ...
  test("Car should be covariant in its type parameter") {
    ...
    val carWithToothpickAndPiano: Car[Cargo] =
      carWithToothpick.add(Piano("Yamaha"))
    carWithToothpickAndPiano.numberOfItems should be(2)
    carWithToothpickAndPiano.item(0) should be(Some(Toothpick("toothpick-1")))
    carWithToothpickAndPiano.item(1) should be(Some(Piano("Yamaha")))
    carWithToothpickAndPiano.item(2) should be(None)
    ...
    val anyCar2: Car[TransportableItem] =
      carWithToothpickAndPiano.add(Pig("Percival"))
    // this compiles, but what is the type of anyCar2 now?

    anyCar2.item(2).get should be(Pig("Percival"))

    assertDoesNotCompile(
      """val anyCar3 = anyCar2.add("Hello World, I am a string")""")
  }
  ...
  test(
    "Car should work as expected with an abstract trait separated out for its API") {

    object Car {
      private class CarImpl[+T <: TransportableItem](items: Vector[T])
          extends Car[T] {
    ...
    assertDoesNotCompile(
      """val anyCar3 = anyCar.add("Hello World, I am a string")""")
  }

  test(
    "A Dock should be able to handle all cargo of a more specific type than it is defined on") {
    ...
    val toothpickShip: Ship[Toothpick] =
      Ship(Seq(Toothpick("tp1"), Toothpick("tp2")))
    ...
    val toothpicksReceived: Seq[Toothpick] =
      transportableItemDock.unload(toothpickShip)
    val livestockReceived: Seq[Livestock] =
      transportableItemDock.unload(mixedLivestockShip)
